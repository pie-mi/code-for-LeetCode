...
public:
    vector<int> dailyTemperatures(vector<int>& temperatures) {
        ...
        {
            if (!higerTemperatures.empty() && temperatures[i] > higerTemperatures.top().first)
            {
                ...
                higerTemperatures.push(make_pair(temperatures[i], i)); //将自己入栈
            }
            else
            {
                higerTemperatures.push(make_pair(temperatures[i], i)); //AI 入栈，先比较再决定是否入栈
            }
